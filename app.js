...
// 这些 ID/元素在 index.html 中已设置：prompt, genBtn, status, progBox, raw, resultBox, saveBtn, copyBtn, dlJsonBtn, dlHtmlBtn
    ...
    while (true) {
      ...
      for (const part of chunks) {
        ...
        try {
          ...
          // ② 同时累积普通 content（有些模型直接把 JSON 写到 content）
          if (delta.content) {
            content += delta.content;
            if (!args && statusEl) statusEl.textContent = '已接收文本 ' + content.length + ' 字符…';
          }
        } catch (_) { /* 单条 data 可能不是 JSON，忽略 */ }
      }
    }
  } catch (e) {
...
function copyRaw () {
  ...
  if (!raw) return;
  navigator.clipboard.writeText(raw).then(()=>{ toast('已复制到剪贴板'); });
}
